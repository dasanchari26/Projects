...
if (isset($_POST['cancel_order_btn'])) {
	header('location:current_order.php'. SID);
	exit();
}
if (isset($_POST['submit_order_btn'])) {
	...
	$query = "SELECT name FROM clothesville_products WHERE productid = $productid LIMIT 1";
	...
	//replacing id
	$query = "UPDATE clothesville_orders SET productid = $new_id WHERE productid = $productid AND username = '$username' AND status = 'checked out'";
	$db->query($query);
	header('location:current_order.php'. SID);
	exit();
}		 				  
...
<html lang="en">
<head>
	<title>ClothesVille - Edit Ordered Product</title>
	<meta charset="utf-8">
	...
	<!-- <div id="wrapper"> -->
		...
		<div class="content"> 
			...
			<?php 
				...
				echo '<br><button name = "submit_order_btn" value = "submit_order_btn" type = "submit" class = "button"><span>Submit Changes</span></button>';
				echo '<button name = "cancel_order_btn" value = "cancel_order_btn" type = "submit" class = "button"><span>Cancel</span></button>';
